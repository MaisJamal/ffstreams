...
def main():
    ...
    for exp in range(counter_exp):
        ...
        if exp >=1 :
            obstacles.append(((obs2_x, 51.75), (5.5, 2.5), (obs2_v, 0)))
        if exp >=2 :
            obstacles.append(((obs3_x, 51.75), (5.5, 2.5), (obs3_v, 0)))
        if exp >=3 :
            obstacles.append(((obs4_x, 51.75), (5.5, 2.5), (obs4_v, 0)))
        if exp >=4 :
            obstacles.append(((obs5_x, 51.75), (5.5, 2.5), (obs5_v, 0)))
        if exp >=5 :
            obstacles.append(((obs6_x, obs6_y),(5.5, 2.5), (obs6_v, 0)))
        
        accelerations = [obsfront_a , obs2_a, obs3_a , obs4_a, obs5_a, obs6_a]

        if exp >=6:
            for add_obs in range(5,exp):
                dist = 7 #m distance between front obstacles
                obstacles.append(((obsfront_x+(exp-add_obs)*dist, 48.25), (5.5, 2.5), (obsfront_v, 0)))
                accelerations.append(obsfront_a)
    ...
    f.close()
    #print("66666666666",obsfront_x,obsfront_v)
    #print(obs2_x,obs2_v, obs2_a)
    #print(obs3_x,obs3_v, obs3_a)
    #print(obs4_x,obs4_v, obs4_a)
    #print(obs5_x,obs5_v, obs5_a)
    #print(obs6_x,obs6_v, obs6_update_time)
    #input("end of exp?")
