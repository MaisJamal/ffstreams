...
def calc_global_paths(fplist, csp):
    for fp in fplist:
        ...
        for i in range(len(fp.s)):
            ######
            if csp == 1:
                ix, iy = fp.s[i] , cfg.wy_middle_lane_center[0]
            else:
                ix, iy = fp.s[i] , cfg.wy_left_lane_center[0]
            #####
...
def calc_ds_l0(y_target,y0): # this is a special case when the road is placed horizontaly 
    if(y_target==cfg.wy_middle_lane_center[0]):
        l0 = y0 - cfg.wy_middle_lane_center[0]
    else:
        l0 = y0 - cfg.wy_left_lane_center[0]
    return l0
...
def get_traj_change_lane(x0,y0,speed0,acc0,curr_dl,curr_ddl,target_y,target_speed):
    global MAX_SPEED
    global MAX_ACCEL
    global MAX_T
    global MIN_T 
...
def get_traj_yield(x0,y0,speed0,acc0,curr_dl,curr_ddl,target_y,target_speed):
    global MAX_SPEED
    global MAX_ACCEL
    global MAX_T
    global MIN_T 
    ...
    MAX_SPEED = cfg.MAX_SPEED  #120.0 / 3.6  # maximum speed [m/s]
    MAX_ACCEL = cfg.MAX_ACC #1  # maximum acceleration [m/ss]

    ob = None 
    

    if (target_y == cfg.wy_middle_lane_center[0]):
        csp = 1
        #print("csp1",stg.wy_middle_lower_lane[0])
    else:
        csp = 2
