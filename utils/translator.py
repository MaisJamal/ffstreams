...
def translate_to_pddl_cr(goal_left,there_is_front_obs,confs,traj_dict,traj_type,traj_array,obstacles,file_name,problem_file_name,obs_traj):
   ...
   # f.write("\t\t(= (at_y q0) 0)\n")
    ...
    if goal_left:
        ...
        f.write("\t(:goal (and (moved_forward) (on_init_lane) ))\n")
...
def translate_to_pddl_apollo(goal_left,there_is_front_obs,confs,traj_dict,traj_type,traj_array,obstacles,file_name,problem_file_name):
    #INIT_SPEED = 20 / 3.6
    #all_y=[stg.wy_middle_lower_lane[0],stg.wy_middle_upper_lane[0]]
    total_time = 10 #seconds
    delta_t = 0.2  # should be even
   ...
   # f.write("\t\t(= (at_y q0) 0)\n")
    ...
    if len(obstacles):
        for i in range(len(obstacles)):
            ...
            obs_start_x = obstacles[i].s
            obs_start_y = obstacles[i].l
            obs_speed = obstacles[i].v
            if obs_speed == 0:
                obs_speed = 0.01
