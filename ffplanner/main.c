...
Token gparse_optimization_backup;
...
{
  gparse_optimization = ( char * ) calloc (20, sizeof( char ) ); 
  CHECK_PTR(gparse_optimization); 
  ...
  if ( gparse_optimization != NULL ) {
    ...
    printf("\n\n is NULL!\n\n");
  char ops_file[2*MAX_LENGTH] = "";
  /* same for fct file 
   */
  char fct_file[2*MAX_LENGTH] = "";
  ...
  PlNode * temp = gorig_initial_facts->sons;
  int count=0;
  while(temp != NULL){
      temp = temp->next;
      count++;
  }
  printf("\n Mais Total no. of initial facts is %d\n",count);

  TypedList * temp_2 = gparse_objects;
  count=0;
  while(temp_2 != NULL){
      temp_2 = temp_2->next;
      count++;
  }
  printf("\n Mais Total no. of objects is %d\n",count);
   ...
   */
  gparse_optimization_backup = "MINIMIZE";
