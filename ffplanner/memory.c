...

{
  ...
  CHECK_PTR(result);
  result->item = (char *) malloc( 100*sizeof (char) );
  CHECK_PTR(result->item);
  result->next = (TokenList * ) malloc( 100*sizeof (TokenList) );
  CHECK_PTR(result->next);
  result->item = NULL; 
  result->next = NULL;
...

{
  ...
  result->item = new_TokenList();
  CHECK_PTR(result->item);
  result->next = (FactList * ) malloc( 100*sizeof (FactList ) );
  CHECK_PTR(result->next);
  result->item = NULL; 
  result->next = NULL;
